...
export async function loadWasm() {
    const go = new Go();
    const result = await WebAssembly.instantiateStreaming(fetch(new URL("../utils.wasm", import.meta.url)), go.importObject);
...
export function toggleCard(id) {
    ...
    if (!card)
        return;
    const body = card.querySelector(".util-card-body");
    if (!body)
        return;
